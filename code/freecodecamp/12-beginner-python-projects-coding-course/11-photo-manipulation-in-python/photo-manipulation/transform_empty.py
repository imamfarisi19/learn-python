...
def adjust_brightness(image, factor):
    ...
    x_pixels, y_pixels, num_channels = image.array.shape # get x, y pixels of image, # channel 
    ...
    for x in range(x_pixels): 
        for y in range(y_pixels): 
            for c in range(num_channels): 
                ...
                # and summing 
                # there is a faster way but this is more straight forward to understand 
                total = 0 
...
def combine_images(image1, image2):
    ...
    # size of image1 and image2 MUST be the same
        x_pixels, y_pixels, num_channels = image1.array.shape # represents x, y pixels of image 
        new_im = Image(x_pixels=x_pixels, y_pixels=y_pixels, num_channels=num_channels) # making a new array to copy values to! 
        for x in range(x_pixels): 
            for y in range(y_pixels): 
                for c in range(num_channels): 
