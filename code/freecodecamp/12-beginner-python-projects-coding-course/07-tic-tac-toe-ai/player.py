...
class GeniusComputerPlayer(Player): 
    ...
    def get_move(self,game): 
        if len(game.available_moves()) == 9:
            ...
            # get the sqare based off the minimax algorithm 
            square = self.minimax(game, self.letter)['position']
        return square 

    def minimax(self, state, player): 
        max_player = self.letter # yourself!! 
        other_player = 'O' if player  == 'X' else 'X' # the other player.. so wathever 
        ...
        for possible_move in state.available_moves(): 
            ...
            sim_score = self.minimax(state, other_player)  # now, we alternative players
            ...
            # step 4: update the dicrtionaries if necessary 
